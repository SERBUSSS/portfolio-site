...
document.addEventListener('DOMContentLoaded', function() {
  ...
  const startFormButton = document.querySelector('#background-form a[href="#process"]');
  ...
  if (startFormButton) {
    startFormButton.addEventListener('click', function(e) {
      e.preventDefault();
  ...
  /**
   * Set up the background animation for fixed positioning
   */
  function initBackgroundAnimation() {
    const backgroundSection = document.getElementById('background-form');
    
    if (backgroundSection) {
      // Create a fixed-position effect for background during form interaction
  ...
  function animateStepTransition(currentIndex, targetIndex, isPrevious = false) {
    ...
    if (isPrevious) {
      ...
      // Make target step visible but off screen
      gsap.set(targetStep, {
        display: 'block',
        opacity: 0,
        y: '-20px',
      ...
      // Animate previous step up from the stack
      tl.to(targetStep, {
        opacity: 1,
        y: 0,
        rotation: 0,
      ...
      // Animate current step down to the stack
      tl.to(currentStep, {
        y: '20px',
        rotation: '5deg',
        scale: 0.9,
