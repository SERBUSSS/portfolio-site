 ...
 * Handles special form interactions including textarea expansion, social media fields,
 * checkbox/radio dependencies, and other interactive elements.
 */
const FormModule = (function() {
    ...
    function init() {
      ...
      // Fix textarea initial states (always do first)
    ...
    /**
     * Fix textarea initial states to prevent auto-focus and overflow issues
     */
    function fixTextareaInitialStates() {
      ...
      // IMPORTANT: Make sure textarea containers are shown initially
    ...
    /**
     * Initialize special field interactions (expandable textareas, social media fields, dependencies)
     */
    ...
    /**
     * Initialize social media field functionality
     */
    ...
    /**
     * Add a new social media field to the form
     */
      ...
      if (typeof gsap !== 'undefined') {
        gsap.to(field, {
          ...
          onComplete: () => {
            ...
            if (socialFieldCount < MAX_SOCIAL_FIELDS && addButton) {
    ...
    /**
     * Initialize checkbox dependencies like Web Design -> Development
     */
    function initializeCheckboxDependencies() {
      ...
      if (webDesignCheckbox && developmentCheckbox) {
        // Clear existing event listeners
        const newWebDesignCheckbox = webDesignCheckbox.cloneNode(true);
        webDesignCheckbox.parentNode.replaceChild(newWebDesignCheckbox, webDesignCheckbox);
        
        // Add new event listener
        newWebDesignCheckbox.addEventListener('change', function() {
          updateDevelopmentCheckboxState(this.checked);
        });
        
        // Initial state
        if (newWebDesignCheckbox.checked) {
      ...
      document.querySelectorAll('input[type="checkbox"]').forEach(checkbox => {
        ...
        if (newCheckbox.checked) {
          updateCheckboxVisualState(newCheckbox);
        }
      });
    }
    
    /**
     * Update the development checkbox state based on web design checkbox
    ...
    function updateDevelopmentCheckboxState(enabled) {
      ...
      if (enabled) {
        ...
        if (container && !container.parentNode.querySelector('.development-tooltip')) {
    ...
    /**
     * Update checkbox visual state (show/hide checkmark)
    ...
    /**
     * Initialize radio button dependencies
     */
    ...
    /**
     * Handle budget option dependency - shows/hides custom budget field
    ...
    function initializeOptionalQuestion() {
      ...
      if (showButton && hideButton && container) {
        ...
        const newHideButton = hideButton.cloneNode(true);
        
        showButton.parentNode.replaceChild(newShowButton, showButton);
        hideButton.parentNode.replaceChild(newHideButton, hideButton);
    ...
    function toggleOptionalQuestion(show) {
      ...
      if (!showButton || !hideButton || !container) return;
    ...
    /**
     * Initialize expandable textareas for better user experience
     */
    function initializeExpandableTextareas() {
        ...
        if (newTextarea.value.trim() !== '') {
            const container = newTextarea.closest('.question-container');
            if (container) {
            const statusElement = container.querySelector('.question-status');
            if (statusElement) {
                statusElement.classList.remove('hidden');
